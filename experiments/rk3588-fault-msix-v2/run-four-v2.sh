...
test "$(grep -h 'RKMESH_V2_PASS' "$R"/node*.log | sed -n 's/.*final=//p' | sort -u)" = '0xc52f003769448c32'
{
  grep 'IVSHMEM_SERVER_READY\|IVSHMEM_ALL_PEERS' "$R/server.log"
  grep -h 'RKMESH_V2_MODULE_LOAD\|RKMESH_V2_READY\|RKMESH_V2_INITIAL_OWNER\|RKMESH_V2_FAULT_SIGNAL\|RKMESH_V2_REQUEST_DOORBELL\|RKMESH_V2_REQUEST_IRQ\|RKMESH_V2_PAGE_TRANSFER\|RKMESH_V2_RESPONSE_DOORBELL\|RKMESH_V2_RESPONSE_IRQ\|RKMESH_V2_ACQUIRE_COMPLETE\|RKMESH_V2_FAULT_RESUMED\|RKMESH_V2_COMMIT\|RKMESH_V2_CHAIN_DONE\|RKMESH_V2_PASS' "$R"/node*.log
