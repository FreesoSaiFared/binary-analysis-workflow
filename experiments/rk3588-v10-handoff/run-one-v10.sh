...
S="/tmp/rkv10-${GITHUB_RUN_ID:-$$}-${RANDOM}.sock"; M="$R/iv.bin"; rm -f "$S"; python3 "$H/ivshmem_server.py" -S "$S" -m "$M" --peers 4 >"$R/server.log" 2>&1 & srv=$!; trap 'kill $srv 2>/dev/null || true; jobs -pr | xargs -r kill 2>/dev/null || true; rm -f "$S"' EXIT
