...
if(st.done!=DONE_MAGIC||st.owner0!=2||st.owner2!=0||st.owner4!=2||st.readers0!=0||st.readers2!=0||st.busy0||st.busy2||st.busy4||st.overlaps!=3||st.delay_proof!=1||st.lease_owner!=3||st.lease_busy||st.page_xfers!=6||st.page_bytes!=24576||st.invalidations!=2||st.lease_xfers!=1||st.lease_bytes!=16384||st.slot2_retries<1||st.final_checksum!=0xf0d496616ca75eecULL)_exit(120);
